...
public class RandomController {
    ...
    @Operation(
            summary = "랜덤뽑기 (RANDOM-01)",
            description = """
                    뽑기 대상 역(is_drawable=true) 중 무작위로 1개를 뽑아 역 정보와 코스 미리보기를 반환한다.
                    - 로그인(X-Member-Id 존재) 시 직전 추천 1건을 제외한다. 제외 후 후보가 없으면 전체에서 다시 뽑는다.
                    - 코스 미리보기는 카테고리별(문화공간/식당/카페/산책) 장소 1개씩으로 구성되며, 장소가 없는 카테고리는 제외된다.
                    - 코스 미리보기는 저장되지 않는다(transient). 저장은 코스 저장 API에서만 일어난다.
                    """
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "뽑기 성공"),
            @ApiResponse(responseCode = "404", description = "뽑기 대상 역이 없음 (`RecommendationErrorCode.NO_DRAWABLE_STATION`)"),
    })
    @PostMapping
