...
public class CustomRecommendationController {
    ...
    @Operation(
            summary = "맞춤추천",
            description = """
                    출발역·이동 가능 시간·여행 스타일(정확히 3개)을 받아 조건에 맞는 역 하나를 추천한다.
                    - 출발역에서 이동 가능 시간 내에 닿는 뽑기 대상 역만 후보로 둔다(상관없음이면 시간 제한 없음).
                    - 여행 스타일 태그로 역 점수(장소 수 합 + 충족 태그 수 × 10)를 매겨 최고점의 90% 이상인 역들을 후보군으로 묶는다.
                    - 후보군에서 안 가본 역(스탬프 없는 역)을 우선한다. 전부 가본 역이면 이 조건은 적용하지 않는다.
